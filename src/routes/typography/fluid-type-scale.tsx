...
function FluidTypeScale() {
	...
	const generateClamp = (stepIndex: number) => {
		...
		const minSize = minBase * Math.pow(minScale, stepIndex);
		const maxSize = maxBase * Math.pow(maxScale, stepIndex);
				...
				<div className="space-y-6 bg-card p-6 border border-border rounded-xl h-fit">
					<div className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-4">
							<div className="space-y-2">
								<Label>Min</Label>
								<Input type="number" value={minView} onChange={e => setMinView(Number(e.target.value))} />
							</div>
							<div className="space-y-2">
								<Label>Max</Label>
								<Input type="number" value={maxView} onChange={e => setMaxView(Number(e.target.value))} />
							</div>
						</div>
					</div>
					...
					<div className="space-y-4 pt-4 border-t border-border">
						...
						<div className="grid grid-cols-2 gap-4">
							<div className="space-y-2">
								<Label>Min</Label>
								<Input type="number" value={minBase} onChange={e => setMinBase(Number(e.target.value))} />
							</div>
							<div className="space-y-2">
								<Label>Max</Label>
								<Input type="number" value={maxBase} onChange={e => setMaxBase(Number(e.target.value))} />
							</div>
						</div>
					</div>
					...
					<div className="space-y-4 pt-4 border-t border-border">
						...
						<div className="grid grid-cols-2 gap-4">
							<div className="space-y-2">
								...
								<Input type="number" step={0.001} value={minScale} onChange={e => setMinScale(Number(e.target.value))} />
							</div>
							<div className="space-y-2">
								<Label>Max Scale</Label>
								<Input type="number" step={0.001} value={maxScale} onChange={e => setMaxScale(Number(e.target.value))} />
							</div>
						</div>
					</div>
				...
				<div className="space-y-8">
					<div className="bg-card border border-border rounded-xl p-8 space-y-6 overflow-hidden">
						{Array.from({ length: steps + 2 }).map((_, idx) => {
							...
							const minSize = Math.round(minBase * Math.pow(minScale, i));
							const maxSize = Math.round(maxBase * Math.pow(maxScale, i));
							return (
								<div key={i} className="flex items-baseline gap-4 w-full">
									...
									<div 
										...
										style={{ fontSize: `clamp(${minSize}px, 2vw, ${maxSize}px)` }}
									>
										The quick brown fox
									</div>
								</div>
					...
					<CodeOutput code={generateCSS()} language="css" label="CSS Variables" />
				</div>
			</div>
		</ToolPageLayout>
	);
